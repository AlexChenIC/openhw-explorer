...
describe("industry landscape", () => {
  it("keeps a focused, source-checkable company map", () => {
    expect(industryCompanies.length).toBeGreaterThanOrEqual(12);
    expect(industryCompanies.length).toBeLessThanOrEqual(20);
    ...
    for (const company of industryCompanies) {
      ...
      expect(company.mark, company.id).toMatch(/^[A-Z]{2,3}$/);
